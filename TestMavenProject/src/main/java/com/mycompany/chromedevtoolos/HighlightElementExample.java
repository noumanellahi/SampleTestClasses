...
public class HighlightElementExample {
	private static void highlightNode(ChromeDevToolsService devToolsService, HighlightConfig highlightConfig,
			String selector) {
		final DOM dom = devToolsService.getDOM();
		final Overlay overlay = devToolsService.getOverlay();

		final Integer nodeId = dom.querySelector(devToolsService.getDOM().getDocument().getNodeId(), selector);
	...
	public static void main(String[] args) {
		...
		ChromeArguments chromeArguments = ChromeArguments.builder().noFirstRun().noDefaultBrowserCheck()
				.disableBackgroundNetworking().disableBackgroundTimerThrottling().disableClientSidePhishingDetection()
				.disableDefaultApps().disableExtensions().disableHangMonitor().disablePopupBlocking()
				.disablePromptOnRepost().disableSync().disableTranslate().metricsRecordingOnly(false)
				.safebrowsingDisableAutoUpdate().headless(false).disableGpu(true).hideScrollbars(true).muteAudio(true)
		...
		network.onLoadingFinished(event -> {
			...
			highlightNode(devToolsService, highlightConfig,
					"body > div.mainContentWrapper > div > div > div > div > article > div > table");
